...
function Video1({ socket, partner, me, videoCallVisible, setVideoCallVisible }: any) {
    ...
    useEffect(() => {
        ...
        if (socket) {
            socket.on("callUser", (data: { from: string, name: any, signal: any }) => {
                ...
                setCaller(data.from);
                setName(data.name?.user_data?.username ?? 'User');
            ...
            socket.on("callEnded", (data: any) => {
                // Logic to handle missed call notification on this user
                console.log(`${data} missed your call.`);
                leaveCall()
            });

            socket.on("callRejected", (data: any) => {
                // Logic to handle missed call notification on this user
                console.log(`${data} rejected your call.`);
                setIsRinging(false);
                leaveCall()
            });
    ...
    const declineCall = () => {
        ...
        socket.emit("rejectCall", { from: me, to: caller, name: userData.email });
    };
    ...
    const leaveCall = () => {
        ...
        if (connectionRef.current) {
            ...
            socket.emit("endCall", { type: userData.type, from: userData.email, to: name });
        }
    };
            ...
            <div className="container">
                ...
                {receivingCall && !callAccepted && (
                    <div className="call-popup" style={{
                        ...
                        <h1 style={{ margin: '10px 0' }}>{name} is calling...</h1>
                        <div style={{
                            display: 'flex',
